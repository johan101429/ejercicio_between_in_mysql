...
CREATE TABLE `clientes` (
  ...
  CONSTRAINT `customers_ibfk_1` FOREIGN KEY (`IdEmpleado`) REFERENCES `Empleados` (`IdEmpleado`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Data for the table `customers` */

insert  into `clientes`(`IdCliente`,`NombreCliente`,`ApellidoContacto`,`NombreContacto`,`Telefono`,`Direccion1`,`Direccion2`,`Ciudad`,`Estado`,`CodigoPostal`,`Pais`,`IdEmpleado`,`LimiteCredito`) values 
...
CREATE TABLE `detallescompra` (
  ...
  `NumeroCompra` smallint(6) NOT NULL,
  PRIMARY KEY (`IdCompra`,`IdProducto`),
  KEY `IdProducto` (`IdProducto`),
  CONSTRAINT `orderdetails_ibfk_1` FOREIGN KEY (`IdCompra`) REFERENCES `compras` (`IdCompra`),
  CONSTRAINT `orderdetails_ibfk_2` FOREIGN KEY (`IdProducto`) REFERENCES `productos` (`IdProducto`)
) ENGINE=InnoDB DEFAULT CHARSET=latin1;

/*Data for the table `orderdetails` */

insert  into `detallescompra`( `IdCompra`,
  `IdProducto`,
  `CantidadOrdenada`,
  `PrecioUnitario`,
  `NumeroCompra`) values 
...
/*___________________ CONSULTAS DE BETWEEN Y IN MYSQL _____________________*/
show tables;
/* 1. Muestra todos los productos cuyo precio de venta sugerido está entre 50 y 100.*/
SELECT * FROM productos WHERE PrecioVentaSugerido BETWEEN 50 AND 100;
...
/*4. Muestra todos los clientes cuyo país están en (USA, France, Singapore).*/
SELECT * FROM clientes WHERE Pais IN ('USA', 'France','Singapore');
...
/*6. Muestra todas las compras cuyos precios unitarios están entre 20 y 50. ok*/
select * from compras join detallescompra on compras.IdCompra = detallescompra.IdCompra where PrecioUnitario between 20 and 50;
/*7. Muestra todos los productos cuyo nombre está en ('1940s Ford truck', '1939
Cadillac Limousine', '1956 Porsche 356A Coupe').*/
select * from productos where nombre in ('1940s Ford truck', '1939
Cadillac Limousine', '1956 Porsche 356A Coupe');
/*8. Muestra todos los clientes cuyo ciudad está en ('NYC', 'Singapore').*/
select * from clientes  where Ciudad in ('NYC', 'Singapore');
/* 9. Muestra todos los clientes cuyo límite de crédito está entre 0.00 y 138500.00
ordenado de mayor a menor.*/
select * from clientes where LimiteCredito between 0.00 and 138500.00 order by LimiteCredito desc;
/*10. Muestra todos los empleados cuyo nombre está en ('Patterson', 'Firrelli').*/
select * from empleados where Apellido in ('Patterson', 'Firrelli');
/*11. Muestra todas las oficinas cuyos países están en ('USA', 'UK').*/
select*from oficinas where Pais in ('USA', 'UK');
/*12. Muestra todas las compras cuyo estatus está en ('Cancelled', 'Resolved').*/
select* from compras where Estatus in ('Cancelled', 'Resolved');
/*13. Muestra todos los clientes cuyo estado está en ('NY', 'CA', 'MA').*/
select * from clientes where Estado in ('NY', 'CA', 'MA');

/*--- Actividad: El aprendiz debe analizar la base de datos y crear 10 caso
adicionales donde use las clausulas BETWEEN e IN .---------------------------------------------*/

select * from compras where FechaEnvio between  '2003-01-09' and '2003-01-14';
 select * from productos where Escala in ('1:10','1:700','1:28');
 select * from clientes where Direccion1 in ('8489 Strong St.','Rue Joseph-Bens 532','120 Hanover Sq.');
 select * from detallescompra where CantidadOrdenada between 30 and 50;
 select * from categorias where IdCategoria in ('Motorcycles','Vintage Cars','Ships');
 select * from pagos where FechaPago between '2004-10-19' and '2005-03-24' order by FechaPago desc ;
select * from productos where IdCategoria in ('Motorcycles','Trucks and Buses');
select * from productos where Cantidad  in ('7933','3975') ;
select * from productos where PrecioVentaSugerido between 53.63 and 95.70;
select* from empleados where Puesto in ('President', 'Sales Rep');
